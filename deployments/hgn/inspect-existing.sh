#!/usr/bin/env bash
# Read-only survey of an existing Plane install, to plan a migration onto this fork.
#
# Writes nothing, changes nothing, stops nothing. Run it on the VPS:
#
#   bash inspect-existing.sh            # assumes the old stack lives in /opt/plane
#   OLD_DIR=/var/plane bash inspect-existing.sh
#
# Hand the whole output back before running any migration steps — the version it
# reports decides whether this is an upgrade, a no-op, or not possible.

set -uo pipefail

OLD_DIR="${OLD_DIR:-/opt/plane}"
API_CONTAINER="${API_CONTAINER:-plane-api-1}"
...
docker inspect "$API_CONTAINER" \
  ...
  || echo "!! container $API_CONTAINER not found — set API_CONTAINER=<name>"
...
if [ -n "${DB_CONTAINER:-}" ]; then
  ...
  hr "Last 5 applied migrations (compare against this fork's 0122)"
